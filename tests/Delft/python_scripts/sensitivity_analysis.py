...
def compare_runs(filenames, labels, fixed_index, sensitive_areas = False, alphas = []):
    ...
    ### plot distances and difference flow caught
...
def change_n_nodes(): ### DEZE HELEMAAL WEGHALEN WANT AL IN COMPARE NUMBER NODES!!!
    year = 2022
...
def change_init_prob(G, n, K, K_i, betas, alpha, C, b, c, B, w):
    ### create new scenarios (only once)
    Bmax = 2
    np.random.seed(0)
    ...
    filenames = [dirname+'\\results_sensitivity_delft\\'+filename for filename in filenames]
...
# G, n, K, K_i, betas, alpha, C, b, c, B, w1 = load_instance.MIP_input(year, MAX_DIST_NODES, random_wind = False)
...
def change_impact_factor(G, n, K, K_i, betas, alpha, C, b, c, B, w):
    Bmax = 2
    G_new = copy.deepcopy(G)
    ### 1. impact factor the same small value everywhere for catching early
    ## create new scenarios (only once)
    ...
    ### 2. impact factor higher values for sensitive areas
    ## create new scenarios (only once)
    ...
    ### 3. impact factor higher value for sensitive edge node
    ## create new scenarios (only once)
    # tests = [1e-10, 1,2,3]
    # tests = [4, 5, 6]
    # node_label = 122 ### most northern node of Delft
    # for impact in tests:
    #     G_new = copy.deepcopy(G)
    #     for index, node in enumerate(G_new.nodes()):
...
filenames = ['results_n308_oldalpha/'+filename for filename in filenames]
