### input: final layer of directed network, in groningen_final_network_exploded_d'+str(MAX_DIST_NODES)+'.geojson
### output: layer with initial distribution, transition probabilities and probability of getting stuck, catching probabilities, in "final_network_nodes_attributes_d"+str(MAX_DIST_NODES)+".geojson"
...
layers_folder = current_directory + "\\QGIS_layers\\"
python_scripts_folder = os.path.normpath(os.path.join(current_directory, '../')) + "\\python_scripts"

import sys
sys.path.insert(1, python_scripts_folder)
import wind_data
...
def get_transition_probabilities(G):
    ...
    attrs = {}
    wind_directions = wind_data.get_wind_directions(2022, wind_groningen = True)
    for node in G.nodes():
...
max_boat_width_path = layers_folder + "max_boat_width_groningen_correct.geojson"
...
water_vegetation_path = layers_folder + "water_vegetation_groningen.geojson"
...
impact_factor_layer_path = layers_folder + "impact_factor_groningen.geojson"
