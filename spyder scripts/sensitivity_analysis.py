...
def change_impact_factor(G, n, K, K_i, betas, alpha, C, b, c, B, w):
    ...
    compare_runs(filenames, labels, fixed_index, sensitive_areas = True, alphas = [np.min((betas*M2)[(betas*M2) > 0]), np.mean((betas*M2)[(betas*M2) > 0]), np.max((betas*M2)[(betas*M2) > 0])])
...
def change_accuracy_types(G, n, K, K_i, betas, alpha, C, b, c, B, w):
    ...
    filenames = ['308nodes.txt', 'catching_accuracy_k2_10percent_308nodes.txt', 'catching_accuracy_k2_20percent_308nodes.txt', 'catching_accuracy_k2_40percent_308nodes.txt', 'catching_accuracy_k2_60percent_308nodes.txt']
    labels = ['estimated', r"a_2 = 0.1", r"a_2 = 0.2", r"a_2 = 0.4", r"a_2 = 0.6"]
    fixed_index = 0
