...
def compare_heuristic(filenames, labels):
    ...
    for filename in filenames:
        with open('heuristic_'+filename) as f:
            run = f.readlines()
    ...
    ax2.set_title("Flow caught heuristic solution/flow caugt exact solution")
    ax2.set_xlabel('budget')
    ax2.set_ylabel('flow caught heuristic/flow caught exact')
...
if __name__ == "__main__":
    ...
    #     load_instance.write_outputs_heuristic(G, n, K, K_i, betas, alpha, C, b, c, B, w, str(n)+'nodes.txt')
    ...
    filenames = ['784nodes_B2.txt', '622nodes_B2.txt', '522nodes_B2.txt', '444nodes_B2.txt']
    ...
    fig = compare_heuristic(filenames, labels)
    fig.savefig('plots/heuristic_fewnodes.png')
...
#%% init prob
    ...
    # load_instance.write_outputs_heuristic(G, n, K, K_i, betas, alpha, C, b_new, c, B, w, 'init_prob_choice10_'+str(n)+'nodes.txt')

    
    # b_new = b*np.random.choice([0.8, 1.2], n)
    # b_new = b_new/np.sum(b_new)
    # load_instance.write_outputs_heuristic(G, n, K, K_i, betas, alpha, C, b_new, c, B, w, 'init_prob_choice20_'+str(n)+'nodes.txt')

    # b_new = b*np.random.choice([0.7, 1.3], n)
    # b_new = b_new/np.sum(b_new)
    # load_instance.write_outputs_heuristic(G, n, K, K_i, betas, alpha, C, b_new, c, B, w, 'init_prob_choice30_'+str(n)+'nodes.txt')
    
    # b_new = np.ones(n)/n
    # load_instance.write_outputs_heuristic(G, n, K, K_i, betas, alpha, C, b_new, c, B, w, 'init_prob_uniform_'+str(n)+'nodes.txt')
    ...
    fig = compare_heuristic(filenames, labels)
    fig.savefig('plots/heuristic_initprob.png')
...
#%% wind
    ...
    #     G, n, K, K_i, betas, alpha, C, b, c, B, w = load_instance.MIP_input(year, MAX_DIST_NODES, random_wind = False)
    #     load_instance.write_outputs_heuristic(G, n, K, K_i, betas, alpha, C, b, c, B, w, 'wind_year'+str(year)+'_'+str(n)+'nodes.txt')

    # G, n, K, K_i, betas, alpha, C, b, c, B, w = load_instance.MIP_input(year, MAX_DIST_NODES, random_wind = True)
    # load_instance.write_outputs_heuristic(G, n, K, K_i, betas, alpha, C, b, c, B, w, 'transition_prob_uniform_'+ str(n)+'nodes.txt')
    ...
    labels = ['year=2020', 'year=2021', 'year=2023', 'transitions uniform']

    fig = compare_heuristic(filenames, labels)
    fig.savefig('plots/heuristic_wind.png')
...
#%% water veg
    ...
    #     load_instance.write_outputs_heuristic(G, n, K, K_i, betas, alpha, C_new, b, c, B, w, 'water_veg_'+str(int(perc*100))+'percent_'+str(n)+'nodes.txt')
    ...
    fig = compare_heuristic(filenames, labels)
    fig.savefig('plots/heuristic_waterveg.png')
